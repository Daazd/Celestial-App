...
# def plot_clusters(X, y_encoded, labels, model_name='Model'):
...
#     plt.title(f'Clusters Visualization (PCA-reduced) for {model_name}')
#     plt.xlabel('PCA Component 1')
#     plt.ylabel('PCA Component 2')
#     plt.grid(True)
#     plt.show()
...
# if __name__ == '__main__':
    ...
    #     # Visualize the clusters after training
    #     logging.info("Visualizing clusters...")
    #     X_transformed = pipeline.named_steps['preprocessor'].transform(X)
    #     plot_clusters(X_transformed, y_encoded=label_encoder.transform(y), labels=label_encoder.classes_)
...
################################# VERSION 2 MODEL IMPROVING MODEL ACCURACY AND PERFORMANCE #################################
...
def extract_features(row):
    ...
    features.update({
        ...
        'character_count': len(description.replace(" ", "")),
        'keyword_count': len(str(row.get('keywords', '')).split(',')) if pd.notna(row.get('keywords')) else 0,
        'has_image': pd.notna(row.get('image_url')),
    ...
    features['text'] = preprocess_text(description or row.get('proper', ''))
    return pd.Series(features)
...
def create_and_train_model(X, y, quick_mode=True):
    ...
    ensemble_clf = VotingClassifier(
        estimators=[('rf', rf_clf), ('gb', gb_clf), ('ada', ada_clf)],
        voting='soft',
        n_jobs=-1
    )
    ...
    param_dist = {
        ...
        'clf__rf__max_depth': randint(10, 100),
        'clf__rf__min_samples_split': randint(2, 20),
        'clf__rf__min_samples_leaf': randint(1, 10),
        'clf__gb__n_estimators': randint(100, 300),
        'clf__gb__max_depth': randint(3, 20),
        'clf__gb__learning_rate': [0.01, 0.05, 0.1, 0.2],
        'clf__ada__n_estimators': randint(50, 200),
        'clf__ada__learning_rate': [0.01, 0.05, 0.1, 0.2]
    }

    cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)
    n_iter = 100 if quick_mode else 200
...
if __name__ == '__main__':
    try:
        X, y = load_and_preprocess_data('celestial_bodies.csv', 'hygdata_v3.csv', max_samples_per_class=1000)
        pipeline, label_encoder, balanced_accuracy = create_and_train_model(X, y, quick_mode=True)
        
        if pipeline:
            save_model(pipeline, label_encoder, 'celestial_model.pkl')
            print(f"Model saved successfully with a balanced accuracy of {balanced_accuracy:.2f}")
            y_encoded = label_encoder.transform(y)
            plot_clusters(X, y_encoded, labels=label_encoder.classes_, model_name='Celestial Model')
        else:
